...
const ProductCate = require("../models/productCateModel");
...
const createCategory = asyncHandler(async (req, res) => {
  try {
    const productCate = new ProductCate(req.body);
    const savedProductCate = await productCate.save();
    res.status(201).json({
      data: savedProductCate,
...
const updateCategort = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const updatedCategory = await ProductCate.findByIdAndUpdate(id, req.body, {
      new: true,
    });
    res.status(201).json({
...
const getCategory = asyncHandler(async (req, res) => {
  ...
  try {
    validateMongoDbId(id);
    const category = await ProductCate.findById(id);
    res.status(202).json({
      data: category,
...
const deleteCategory = asyncHandler(async (req, res) => {
  ...
  try {
    validateMongoDbId(id);
    const deletedCategory = await ProductCate.findByIdAndDelete(id);
    res
      .status(203)
